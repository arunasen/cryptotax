...
BEGIN
	...
	SELECT asset_basis_remaining, usd_amount-basis_used as basisUSD, transaction_id, date
	FROM transactions
	WHERE asset =  coin
	and transaction_type in ('income', 'buy')
	and basis_used < usd_amount
    and usd_amount-basis_used > 0.009
    ORDER BY date;
	...
	sellingLoop: LOOP
        ...
        buyingLoop: LOOP
			...
			UPDATE transactions
			SET basis_used = usd_used_buy + basis_used, asset_basis_remaining = buy_amount
